...
siteb_plot <- list()
AICdf <- data.frame(site = sites, Numbers = NA, llBPL = NA, lllognorm = NA, AICBPL = NA, AIClognorm = NA)
...
for(i in 1:nsites){
  ...
  AICdf$Numbers[i] <- length(sitedata$aboveGroundLiveBiomass_kilograms)
...
pdf("qqplots_trees.pdf", width = 12, height = 6)
for(i in 1:nsites){
  par(mfrow=c(1,2))
